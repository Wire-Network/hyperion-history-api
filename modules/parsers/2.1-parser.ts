...
export default class HyperionParser extends BaseParser {
    ...
    public async parseMessage(worker: MainDSWorker, messages: Message[]): Promise<void> {
        ...
        for (const message of messages) {
            ...
            // deserialize result using wirejs (faster)
            ds_times.wirejs.result = timedFunction(dsProfiling, () => {
            ...
            if (res.block && res.block.length) {
                ...
                if (typeof res.block === 'object' && res.block.length === 2) {
                    ...
                    if (!block) {
                        try {
                            // deserialize signed_block using wirejs
                            ds_times.wirejs.signed_block = timedFunction(dsProfiling, () => {
                                block = deserialize('signed_block_variant', res.block, this.txEnc, this.txDec, worker.types);
                            });
                        } catch (e) {
                            hLog('signed_block_variant deserialization failed with wirejs!');
                        }
                    }
                }
            ...
            if (allowProcessing && res.traces && res.traces.length) {
                ...
                // deserialize transaction_trace using wirejs
                if (!traces) {
                    try {
                        ds_times.wirejs.transaction_trace = timedFunction(dsProfiling, () => {
                            traces = deserialize('transaction_trace[]', res.traces, this.txEnc, this.txDec, worker.types);
                        });
                    } catch (e) {
                        hLog('transaction_trace[] deserialization failed with wirejs!');
                    }
                }
            ...
            if (allowProcessing && res.deltas && res.deltas.length) {
                ...
                // deserialize table_delta using wirejs
                if (!deltas) {
                    try {
                        ds_times.wirejs.table_delta = timedFunction(dsProfiling, () => {
                            deltas = deserialize('table_delta[]', res.deltas, this.txEnc, this.txDec, worker.types);
                        });
                    } catch (e) {
                        hLog('table_delta[] deserialization failed with wirejs!');
                    }
                }
            ...
            if (worker.conf.settings.ds_profiling) {
                ...
                line.push(...[ds_times.wirejs.result, ds_times.wirejs.signed_block, ds_times.wirejs.transaction_trace, ds_times.wirejs.table_delta]);
