...
describe('JsSignatureProvider', () => {
    ...
    describe('secp256k1 elliptic', () => {
        it('generates a private and public key pair', () => {
            const { privateKey, publicKey } = (0, eosjs_key_conversions_1.generateKeyPair)(eosjs_numeric_1.KeyType.k1, {
                secureEnv: true,
            });
        ...
        it('throws error with no options.secureEnv variable', () => {
            expect(() => (0, eosjs_key_conversions_1.generateKeyPair)(eosjs_numeric_1.KeyType.k1)).toThrowError();
        });
        it('Retrieves the public key from a private key', () => {
            const privateKey = eosjs_key_conversions_1.PrivateKey.fromString(privateKeys[0]);
            const publicKey = privateKey.getPublicKey();
            expect(publicKey.toString()).toEqual(k1FormatPublicKeys[0]);
        });
        it('builds public keys from private when constructed', async () => {
            const provider = new eosjs_jssig_1.JsSignatureProvider(privateKeys);
        ...
        it('confirm elliptic conversion functions are actually reciprocal', async () => {
            ...
            const sig = eosjs_key_conversions_1.Signature.fromString(signOutput.signatures[0]);
            const ellipticSig = sig.toElliptic();
            const eosSig = eosjs_key_conversions_1.Signature.fromElliptic(ellipticSig, eosjs_numeric_1.KeyType.k1);
            expect(sig).toEqual(eosSig);
        });
        it('verify a transaction', async () => {
            const provider = new eosjs_jssig_1.JsSignatureProvider([privateKeys[0]]);
            const chainId = '12345';
            ...
            const signature = eosjs_key_conversions_1.Signature.fromString(signOutput.signatures[0]);
        ...
        it('ensure public key functions are actual inverses of each other', async () => {
            const eosioPubKey = eosjs_key_conversions_1.PublicKey.fromString(k1FormatPublicKeys[0]);
            const ellipticPubKey = eosioPubKey.toElliptic();
            const finalEosioKeyAsK1String = eosjs_key_conversions_1.PublicKey.fromElliptic(ellipticPubKey, eosjs_numeric_1.KeyType.k1).toString();
        ...
        it('verify that PUB_K1_ and Legacy pub formats are consistent', () => {
            ...
            const ellipticPubKey = eosjs_key_conversions_1.PublicKey.fromString(eosioLegacyPubKey).toElliptic();
        ...
        it('verify that privateKey toLegacyString() and toString() are consistent', () => {
            const privKeyFromK1 = eosjs_key_conversions_1.PrivateKey.fromString(privateKeysK1[0]);
            const privKeyFromLegacy = eosjs_key_conversions_1.PrivateKey.fromString(privateKeys[0]);
        ...
        it('verify that publicKey toLegacyString() and toString() are consistent', () => {
            const pubKeyFromK1 = eosjs_key_conversions_1.PublicKey.fromString(k1FormatPublicKeys[0]);
            const pubKeyFromLegacy = eosjs_key_conversions_1.PublicKey.fromString(legacyPublicKeys[0]);
        ...
        it('ensure private key functions are actual inverses of each other', async () => {
            ...
            const privEosioKey = eosjs_key_conversions_1.PrivateKey.fromString(priv);
            const privEllipticKey = privEosioKey.toElliptic();
            const finalEosioKeyAsString = eosjs_key_conversions_1.PrivateKey.fromElliptic(privEllipticKey, eosjs_numeric_1.KeyType.k1).toString();
        ...
        it('verify that public key validate function correctly assesses public keys', () => {
            const publicKey = eosjs_key_conversions_1.PublicKey.fromString(k1FormatPublicKeys[0]);
            expect(publicKey.isValid()).toEqual(true);
        });
        it('Ensure elliptic sign, recover, verify flow works', () => {
            const KPrivStr = privateKeys[0];
            const KPriv = eosjs_key_conversions_1.PrivateKey.fromString(KPrivStr);
            const dataAsString = 'some string';
            const ellipticHashedString = (0, eosjs_key_conversions_1.sha256)(dataAsString);
    ...
    describe('p256 elliptic', () => {
        it('generates a private and public key pair', () => {
            const { privateKey, publicKey } = (0, eosjs_key_conversions_1.generateKeyPair)(eosjs_numeric_1.KeyType.r1, {
                secureEnv: true,
            });
            expect(privateKey).toBeInstanceOf(eosjs_key_conversions_1.PrivateKey);
            expect(privateKey.isValid()).toBeTruthy();
        ...
        it('throws error with no options.secureEnv variable', () => {
            expect(() => (0, eosjs_key_conversions_1.generateKeyPair)(eosjs_numeric_1.KeyType.r1)).toThrowError();
        });
        it('throws error when attempting a legacy private key from r1 format', () => {
            const privateKey = eosjs_key_conversions_1.PrivateKey.fromString(privateKeysR1[0]);
        ...
        it('throws error when attempting a legacy public key from r1 format', () => {
            const publicKey = eosjs_key_conversions_1.PublicKey.fromString(r1FormatPublicKeys[0]);
        ...
        it('builds public keys from private when constructed', async () => {
            const provider = new eosjs_jssig_1.JsSignatureProvider(privateKeysR1);
            const actualPublicKeys = await provider.getAvailableKeys();
        ...
        it('confirm elliptic conversion functions are actually reciprocal', async () => {
            ...
            const sig = eosjs_key_conversions_1.Signature.fromString(signOutput.signatures[0]);
            const ellipticSig = sig.toElliptic();
            const eosSig = eosjs_key_conversions_1.Signature.fromElliptic(ellipticSig, eosjs_numeric_1.KeyType.r1);
            expect(sig).toEqual(eosSig);
        });
        it('verify a transaction', async () => {
            const provider = new eosjs_jssig_1.JsSignatureProvider([privateKeysR1[0]]);
            const chainId = '12345';
        ...
        it('ensure public key functions using p256 format are actual inverses of each other', async () => {
            const eosioPubKey = eosjs_key_conversions_1.PublicKey.fromString(r1FormatPublicKeys[0]);
            const ellipticPubKey = eosioPubKey.toElliptic();
            const finalEosioKeyAsR1String = eosjs_key_conversions_1.PublicKey.fromElliptic(ellipticPubKey, eosjs_numeric_1.KeyType.r1).toString();
        ...
        it('ensure private key functions using p256 format are actual inverses of each other', async () => {
            ...
            const privEosioKey = eosjs_key_conversions_1.PrivateKey.fromString(priv);
            const privEllipticKey = privEosioKey.toElliptic();
            const finalEosioKeyAsString = eosjs_key_conversions_1.PrivateKey.fromElliptic(privEllipticKey, eosjs_numeric_1.KeyType.r1).toString();
            expect(privEosioKey.toString()).toEqual(finalEosioKeyAsString);
        });
        it('verify that public key validate function correctly assesses public keys', () => {
            const publicKey = eosjs_key_conversions_1.PublicKey.fromString(r1FormatPublicKeys[0]);
