...
const serializedTx = [
    41, 210, 142, 91, 208, 148, 139, 46, 31, 30, 0, 0, 0, 0, 2, 48, 21, 164, 25,
    83, 149, 177, 202, 0, 0, 0, 87, 45, 60, 205, 205, 1, 0, 128, 138, 81, 125,
    195, 84, 203, 0, 0, 0, 0, 168, 237, 50, 50, 0, 48, 21, 164, 25, 83, 149,
    177, 202, 0, 0, 0, 87, 45, 60, 205, 205, 1, 0, 128, 138, 81, 125, 195, 84,
    203, 0, 0, 0, 0, 168, 237, 50, 50, 0, 0,
];
...
describe('eosjs-api', () => {
    ...
    beforeEach(() => {
        rpc = new eosjs_jsonrpc_1.JsonRpc('', { fetch });
    ...
    describe('Abi Serialization', () => {
        ...
        it('deserializes/serializes the 1.0 abi', () => {
            const raw = (0, eosjs_numeric_1.base64ToBinary)(serializedAbis['1.0']);
        ...
        it('deserializes/serializes the 1.1 abi', () => {
            const raw = (0, eosjs_numeric_1.base64ToBinary)(serializedAbis['1.1']);
            const deserializedAbi = api.rawAbiToJson(raw);
            const serializedAbi = api.jsonToRawAbi(deserializedAbi);
            expect(serializedAbi).toEqual(raw);
        });
        it('deserializes/serializes the 1.2 abi', () => {
            const raw = (0, eosjs_numeric_1.base64ToBinary)(serializedAbis['1.2']);
            const deserializedAbi = api.rawAbiToJson(raw);
            const serializedAbi = api.jsonToRawAbi(deserializedAbi);
            expect(serializedAbi).toEqual(raw);
        });
    ...
    describe('Api shorthand design (JsonAbi)', () => {
        ...
        it('generates the same serialized data using the longer authorization', async () => {
            await api.getAbi('testeostoken');
            const firstSerializedAction = api
                .with('testeostoken')
                .as('thegazelle')
                .transfer('thegazelle', 'remasteryoda', '1.0000 EOS', 'For a secure future.');
            const secondSerializedAction = api
                .with('testeostoken')
                .as([{ actor: 'thegazelle', permission: 'active' }])
                .transfer('thegazelle', 'remasteryoda', '1.0000 EOS', 'For a secure future.');
        ...
        it('confirms the transaction extension serialization is reciprocal', async () => {
            ...
            const serialized = [
                [
                    1,
                    '0000000080ABBCA90010000000000000FA000000000000000000000000000000',
                ],
            ];
            const deserialized = {
                resource_payer: {
