 ...
 * @param size bignum size (bytes)
 */
const signedDecimalToBinary = (size, s) => {
    const negative = s[0] === '-';
    const modifiedString = negative ? s.slice(1) : s;
    const result = (0, exports.decimalToBinary)(size, modifiedString);
    if (negative) {
 ...
 * @param minDigits 0-pad result to this many digits
 */
const signedBinaryToDecimal = (bignum, minDigits = 1) => {
 ...
 * @param minDigits 0-pad result to this many digits
 */
const binaryToBase58 = (bignum, minDigits = 1) => {
    const result = [];
...
exports.digestSuffixRipemd160 = digestSuffixRipemd160;
const stringToKey = (s, type, size, suffix) => {
    ...
    const digest = new Uint8Array((0, exports.digestSuffixRipemd160)(result.data, suffix));
...
const keyToString = (key, suffix, prefix) => {
    const digest = new Uint8Array((0, exports.digestSuffixRipemd160)(key.data, suffix));
...
const stringToPublicKey = (s) => {
    ...
    if (s.substr(0, 3) === 'EOS') {
        const whole = (0, exports.base58ToBinary)(exports.publicKeyDataSize + 4, s.substr(3));
        const key = {
            type: KeyType.k1,
        ...
        return stringToKey(s.substr(7), KeyType.em, exports.publicKeyDataSize, 'EM');
    }
    else {
...
const publicKeyToLegacyString = (key) => {
    if ((key.type === KeyType.k1 || key.type === KeyType.em) &&
...
const publicKeyToString = (key) => {
    if (key.type === KeyType.k1 && key.data.length === exports.publicKeyDataSize) {
        return keyToString(key, 'K1', 'PUB_K1_');
    }
    else if (key.type === KeyType.r1 &&
        key.data.length === exports.publicKeyDataSize) {
    ...
    else if (key.type === KeyType.em) {
        return keyToString(key, 'EM', 'PUB_EM_');
    }
    else {
        throw new Error('unrecognized public key format');
    }
 ...
 */
const convertLegacyPublicKey = (s) => {
    if (s.substr(0, 3) === 'EOS') {
 ...
 * Leaves other formats untouched
 */
const convertLegacyPublicKeys = (keys) => {
...
const privateKeyToLegacyString = (key) => {
    if (key.type === KeyType.k1 && key.data.length === exports.privateKeyDataSize) {
        ...
        const digest = new Uint8Array((0, hash_js_1.sha256)().update((0, hash_js_1.sha256)().update(whole).digest()).digest());
...
const stringToSignature = (s) => {
    ...
    else if (s.substr(0, 7) === 'SIG_EM_') {
        return stringToKey(s.substr(7), KeyType.em, exports.signatureDataSize, 'EM');
        // return emStringToKey(s.substr(7), signatureDataSize, 'EM');
    }
    else {
...
const signatureToString = (signature) => {
    ...
    else if (signature.type === KeyType.em) {
        return keyToString(signature, 'EM', 'SIG_EM_');
    }
    else {
        throw new Error('unrecognized signature format');
    }
