...
var supportedAbiVersion = function (version) {
    return (version.startsWith("sysio::abi/1.") || version.startsWith("eosio::abi/1."));
};
...
var createInitialTypes = function () {
    var result = new Map(Object.entries({
        ...
        bitset: createType({
            ...
            serialize: function (buffer, data) {
                ...
                for (var byteIdx = 0; byteIdx < numBytes; byteIdx++) {
                    ...
                    for (var bitIdx = 0; bitIdx < 8; bitIdx++) {
                        var bitPos = byteIdx * 8 + bitIdx;
                        if (bitPos < numBits) {
                            var charIdx = numBits - bitPos - 1;
                            if (data[charIdx] === "1") {
                                byte |= 1 << bitIdx;
                            }
                        }
            ...
            deserialize: function (buffer) {
                var numBits = buffer.getVaruint32();
                if (numBits === 0)
                    return "";
                var numBytes = Math.ceil(numBits / 8);
                var bytes = [];
...
//# sourceMappingURL=wirejs-serialize.js.map